...
#! Files for training
train_file_prefix = "emg_data/Day2/26092024_FW28D_"
target_file_prefix = ["quali_data/FW28D/Day2/quali_torque_elbow_", "quali_data/FW28D/Day2/quali_torque_front_", "quali_data/FW28D/Day2/quali_torque_side_"]
...
mvc = mvc = 2.7579163508176626e-06
delay = 25
beta1 = 0.25
beta2 = 0.05
gamma = 0.7
...
save_fig = True
suffix = "try_4"
plot_folder = "FW28D_plot_" + suffix
...
for typ_idx in range(len(mov_type_d)):
    for set_idx in range(len(set_num_d)):
        # print(pathlib.Path(data_path + train_file_prefix + weights_d[0] + '_' + mov_type_d[typ_idx] + '_set' + set_num_d[set_idx] + '.npy'))
        #! Check if the file exists
        if pathlib.Path(data_path + train_file_prefix + weights_d[0] + '_' + mov_type_d[typ_idx] + '_set' + set_num_d[set_idx] + '.npy').is_file():
            ...
            EMG_Data = EEGData(format = "Recorded_LSL_stream", filenames = [train_file_prefix + weights_d[0] + '_' + mov_type_d[typ_idx] + '_set' + set_num_d[set_idx]], data_path = data_path, f_samp=f_samp, channel_names=channel_names_i, add_marker_channel = False)
            print(EMG_Data.data.shape)
            # EMG_Data.data = EMG_Data.data[:-2,:]
            # print(EMG_Data.data.shape)
            ...
            sos_bp = EMG_Data.designFilter(f_low = 245.0, f_high = 20, order = 2, filter_type = "scipy_butter", return_type = "sos")
            sos_lp = EMG_Data.designFilter(f_low = 5.0, f_high = None, order = 2, filter_type = "scipy_butter", return_type = "sos")

            #! Windowing the filtered data
            _ = EMG_Data.windowContinuousData(startmarkernumber = 1, stopmarkernumber = 1, window_size = window_size_x, window_step = window_step_x, start_index_offset = 0, start_channel_pick=0, end_channel_pick=8,return_window_end_indices = True)
            ...
            EMG_Data.filterWindows(sos = sos_bp, apply_method = "zero_phase_sos", padtype = "even")

            EMG_Data.windows = np.abs(EMG_Data.windows)

            # plt.figure()
            # plt.plot(EMG_Data.getWindows()[0,2,:,5])
            # plt.show()

            EMG_Data.filterWindows(sos = sos_lp, apply_method = "zero_phase_sos", padtype = "even")

            EMG_Data.calcCalibStats()
            _,_,_, max_val = EMG_Data.getCalibStats()

            mvc_new = np.vstack([mvc_new, np.expand_dims(max_val, axis=0)])

            for i in range(EMG_Data.windows.shape[1]):
                EMG_Data.windows[:,i,:,:] = EMG_Data.windows[:,i,:,:]/max_val[i]

            # plt.figure()
            # plt.plot(EMG_Data.getWindows()[0,2,:,5])
            # plt.show()
            
            EMG_Data.calculateActivationForceFunctionWindows(d=25, b1=0.75, b2=0.05, g=0.1, nonlinear_shape_factor=-1.5)
            ...
            continue
mvc_final = np.zeros([8])
for i in range(8):
    mvc_final[i] = np.max(mvc_new[:,i])
print(f"Max vals of each channel: {mvc_final}")
# **********************************************************************************
# *************************** Train, load or test Model ****************************
# **********************************************************************************

#! Init model with norm layer
model_e = AAN_Model(neurons_inp=neurons_inp, act_inp=act_inp, neurons_h1=neurons_h1, act_h1=act_h1, neurons_h2=neurons_h2, act_h2=act_h2, inp_data=x_train_combined, use_norm_layer=True)
MLP_model_e = MLModel(model = model_e, type= "keras")

model_f = AAN_Model(neurons_inp=neurons_inp, act_inp=act_inp, neurons_h1=neurons_h1, act_h1=act_h1, neurons_h2=neurons_h2, act_h2=act_h2, inp_data=x_train_combined, use_norm_layer=True)
MLP_model_f = MLModel(model = model_f, type= "keras")

model_s = AAN_Model(neurons_inp=neurons_inp, act_inp=act_inp, neurons_h1=neurons_h1, act_h1=act_h1, neurons_h2=neurons_h2, act_h2=act_h2, inp_data=x_train_combined, use_norm_layer=True)
MLP_model_s = MLModel(model = model_s, type= "keras")

#! Train model
print("Training MLP model for elbow joint...") 
MLP_model_e.trainModel(save_trained_model = should_save_model, model_filename =data_path+"zdemo_ml_models/FW28D/Day2/"+subject+"_AAN_model_elbow"+"_"+suffix, train_epochs= n_epochs, batch_size=n_batch_size, class_weights=None, x_train=x_train_combined, y_train= y_e_train_combined[:,0], validation_split=validation_split, loss_fcn=loss_fcn, optimizer=optimizer,metrics=metrics, show_train_results=False, callbacks=early_callback)
print("MLP training for elbow done !!\n")

print("Training MLP model for shoulder front joint...") 
MLP_model_f.trainModel(save_trained_model = should_save_model, model_filename =data_path+"zdemo_ml_models/FW28D/Day2/"+subject+"_AAN_model_front"+"_"+suffix, train_epochs= n_epochs, batch_size=n_batch_size, class_weights=None, x_train=x_train_combined, y_train= y_f_train_combined[:,0], validation_split=validation_split, loss_fcn=loss_fcn, optimizer=optimizer,metrics=metrics, show_train_results=False, callbacks=early_callback)
print("MLP training for front done !!\n")

print("Training MLP model for shoulder side joint...") 
MLP_model_s.trainModel(save_trained_model = should_save_model, model_filename =data_path+"zdemo_ml_models/FW28D/Day2/"+subject+"_AAN_model_side"+"_"+suffix, train_epochs= n_epochs, batch_size=n_batch_size, class_weights=None, x_train=x_train_combined, y_train= y_s_train_combined[:,0], validation_split=validation_split, loss_fcn=loss_fcn, optimizer=optimizer,metrics=metrics, show_train_results=False, callbacks=early_callback)
...
dir_d = pathlib.Path("../../plots/m-rock_demo/Day2/" + plot_folder)
...
plt.grid()
if save_fig:
    plt.savefig("../../plots/m-rock_demo/Day2/" + plot_folder +"/test_elbow_median.png")
...
plt.grid()
if save_fig:
    plt.savefig("../../plots/m-rock_demo/Day2/" + plot_folder +"/test_front_median.png")
...
plt.grid()
if save_fig:
    plt.savefig("../../plots/m-rock_demo/Day2/" + plot_folder +"/test_side_median.png")
...
plt.grid()
if save_fig:
    plt.savefig("../../plots/m-rock_demo/Day2/" + plot_folder +"/test_elbow_raw.png")
...
plt.grid()
if save_fig:
    plt.savefig("../../plots/m-rock_demo/Day2/" + plot_folder +"/test_front_raw.png")
...
plt.grid()
if save_fig:
    plt.savefig("../../plots/m-rock_demo/Day2/" + plot_folder +"/test_side_raw.png")
