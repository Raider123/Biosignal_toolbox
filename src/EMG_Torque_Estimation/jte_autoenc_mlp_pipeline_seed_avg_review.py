...
from biosignal_toolbox.models.AANModel import AAN_Model, Autoencoder
...
end_init = perf_counter_ns()
print(f"Initialisation time: {end_init - start_init} ns")
...
for wgt_idx, wgt in enumerate(weights):
    for mov_idx, mov in enumerate(mov_types):
        ...
        Quali_Data_Side = EEGData(format="NumpyQualisys", 
                                  ...
                                  channel_names=cfg.preprocess_param.channel_names_quali, 
                                  add_marker_channel=True)
        end_obj_init = perf_counter_ns()
        print(f"Object initialisation time: {end_obj_init - start_obj_init} ns")
        ...
        #! ************************************************
        #? High pass filter
        start_bpfilter = perf_counter_ns()
        ...
        EMG_Data.filterData_offline(filter_method=cfg.preprocess_param.filter_method, 
                                    sos=sos_hp)
        end_bpfilter = perf_counter_ns()
        print(f"Bandpass filter time: {end_bpfilter - start_bpfilter} ns")
        ...
        EMG_Data.applyVarianceFilter_data(ring_buffer=ring_buffer, 
                                          width=width, 
                                          index=index)
        end_var = perf_counter_ns()
        print(f"Variance filter time: {end_var - start_var} ns")
        ...
        end_lpfilter = perf_counter_ns()
        print(f"Lowpass filter time: {end_lpfilter - start_lpfilter} ns")
        ...
        #     end_act = perf_counter_ns()
        #     print(f"Activation function time: {end_act - start_act} ns")
