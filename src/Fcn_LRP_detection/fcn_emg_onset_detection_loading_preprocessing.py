...
def EMGProcessingPipeline(dataset_emg, dataset_eeg):
    ...
    # loading EMG data 
    emg_data, emg_time_axis, emg_ch_names = emg_lib.loadCometaEMGData(dataset_emg)
...
emg_set1_processed, time_axis_down, emg_ch_names_select = EMGProcessingPipeline(dataset_EMG[0], [dataset_EEG[0]])
emg_set2_processed, time_axis_down, emg_ch_names_select = EMGProcessingPipeline(dataset_EMG[1], [dataset_EEG[1]])
emg_set3_processed, time_axis_down, emg_ch_names_select = EMGProcessingPipeline(dataset_EMG[2], [dataset_EEG[2]])
...
for iterations in set_nums:  # change here later on 
    ...
    np.save(data_path+subject_paradigm_name+"_train_"+str(iterations), emg_epochs_train_scaled)
    ...
    np.save(data_path+"time_axis_emg_epochs", time_axis_down)
    np.save(data_path+"remaining_emg_channel_names", emg_ch_names_select)
