...
#EEGNet-parameter  (only for naming convention)
dropoutRate = 0.25
kernLength = 60
F1 = 8
D = 2
...
# Gather all the sets into one list
all_sets = []
for dataset_key, sets in datasets.items():
    if dataset_key == 'XP01':
        # Merge XP01 set3 and set4 together and add them as one set
        set3_and_4 = ','.join([datasets[dataset_key][2], datasets[dataset_key][3]])
        all_sets.extend([datasets[dataset_key][0], datasets[dataset_key][1], set3_and_4])
    else:
        all_sets.extend(sets)

# The number of all sets
n_sets = len(all_sets)

# Results folder path
base_folder_path = "C:/Users/Chrissy/Desktop/Results/" + str(dropoutRate) + "," + str(kernLength) + "," + str(
    F1) + "," + str(D) + "," + str(F2) + "_" + date + "_" + time + "_SCN_dp" + "/"
...
avg_train_loss = {}
    ...
    # Loop over each set treating it as the test set
for i in range(n_sets):
    test_set = all_sets[i]
    train_val_sets = np.concatenate((all_sets[:i], all_sets[i + 1:]))

    # Create a folder for each dataset
    dataset_name = test_set.split("/")[-1].split("_")[2]
    ...
    combination_name = f'{dataset_name}_combination_{i + 1}'
    ...
    raw_data_train_val = []
    for dataset in train_val_sets:
        if ',' in dataset:  # XP01 set3 and set4 will be a comma-separated string
            dataset_paths = dataset.split(',')
            raw1 = io.read_raw_brainvision(dataset_paths[0], preload=True, verbose=False)
            raw2 = io.read_raw_brainvision(dataset_paths[1], preload=True, verbose=False)
            raw = mne.concatenate_raws([raw1, raw2])
        else:
            raw = io.read_raw_brainvision(dataset, preload=True, verbose=False)

        raw_data_train_val.append(raw)

    # Concatenate all raw objects
    raw_data_train_val = mne.concatenate_raws(raw_data_train_val)

    # Preprocess the training and validation data
    wind_arr_train_val = preprocess(raw_data_train_val)
    # Load and preprocess the raw data for testing
    if ',' in test_set:
        dataset_paths = test_set.split(',')
        raw1 = io.read_raw_brainvision(dataset_paths[0], preload=True, verbose=False)
        raw2 = io.read_raw_brainvision(dataset_paths[1], preload=True, verbose=False)
        raw_data_test = mne.concatenate_raws([raw1, raw2])
    else:
        raw_data_test = io.read_raw_brainvision(test_set, preload=True, verbose=False)
    ...
    test_arr = wind_arr_test  # All data in the remaining set is for testing
    ...
    model = ShallowConvNet(2, Chans=chans, Samples=samples, dropoutRate=0.25)
    ...
    checkpointer = ModelCheckpoint(
        filepath="/tmp/checkpoint/" + str(dropoutRate) + "," + str(kernLength) + "," + str(F1) + "," + str(
            D) + "," + str(F2) + "/" + dataset_name + "_SCN_dp" + '.h5', verbose=1,
        save_best_only=True)
    ...
    model.load_weights("/tmp/checkpoint/" + str(dropoutRate) + "," + str(kernLength) + "," + str(F1) + "," + str(D) + "," + str(F2) + "/" + dataset_name + "_SCN_dp" + '.h5')
...
results_df.to_csv(os.path.join(base_folder_path, "results_SCNdp.csv"), index=False, sep=';')
