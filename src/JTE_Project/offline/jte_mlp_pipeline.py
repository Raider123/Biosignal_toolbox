...
for wgt_idx, wgt in enumerate(weights):
    for mov_idx, mov in enumerate(mov_types):
        ...
        # Check if EMG files exist for this weight-movement combination
        if not emg_table[wgt_idx][mov_idx]:
            print(f"WARNING: Keine EMG-Dateien für weight={wgt}, movement={mov} vorhanden! Überspringe diese Kombination.")
            continue
        
        # Check if Quali files exist for this weight-movement combination
        if not quali_e_table[wgt_idx][mov_idx] or not quali_sf_table[wgt_idx][mov_idx] or not quali_ss_table[wgt_idx][mov_idx]:
            print(f"WARNING: Nicht alle Quali-Dateien für weight={wgt}, movement={mov} vorhanden! Überspringe diese Kombination.")
            continue
        ...
        weights_code += [wgt_idx] * EMG_Data.getWindows().shape[3]
        mov_code += [mov_idx] * EMG_Data.getWindows().shape[3]
        ...
        n_cycles[0] = 3
        n_cycles[1] = 4
        mwc_feature = EMG_Data_freq.getMorletWaveletCoeffFeatures_windows(freqs=freqs, n_cycles=n_cycles)
        ...
        X_train_cat_temp, X_test_cat= train_test_split(x_cat, train_size=cfg.model_param.train_test_split, shuffle=False)

        X_train_cat, X_val_cat, = train_test_split(X_train_cat_temp, train_size= 1 - cfg.model_param.validation_split, shuffle=False)
        ...
        history_len = 1
        X_train, Y_train, meta_train = EMG_Data.stackHistoryCatMeta_windows(x_num=X_train, y_num=Y_train, x_cat=X_train_cat, history_len=history_len, wgt=wgt, mov=mov)

        X_test, Y_test, meta_test = EMG_Data.stackHistoryCatMeta_windows(x_num=X_test, y_num=Y_test, x_cat=X_test_cat, history_len=history_len, wgt=wgt, mov=mov)

        X_val, Y_val, meta_val = EMG_Data.stackHistoryCatMeta_windows(x_num=X_val, y_num=Y_val, x_cat=X_val_cat, history_len=history_len, wgt=wgt, mov=mov)
...
joblib.dump(scaler_to_save, getAbsolutePath("src/JTE_Project/online/resources/y_scaler/Y_scaler.pkl"))
...
# Define all possible weight and movement categories explicitly
encoder_wgt = OneHotEncoder(sparse_output=False, categories=[weights])
...
encoder_mov = OneHotEncoder(sparse_output=False, categories=[mov_types])
...
print(f"Weight one-hot encoded shape: {wgt_test_onehot.shape} (expected: {len(weights)} categories)")
print(f"Movement one-hot encoded shape: {mov_test_onehot.shape} (expected: {len(mov_types)} categories)")
...
if cfg.model_param.load_models == False:
    ...
    save_model_path = getAbsolutePath("data/jte/ml_models/BU62D")
    model = load_model(os.path.join(save_model_path, "mlp_model_4.keras"), compile=False)
