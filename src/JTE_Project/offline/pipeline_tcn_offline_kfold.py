...
for wgt_idx, wgt in enumerate(weights):
    for mov_idx, mov in enumerate(mov_types):
        ...
        if cfg.settings.advanced_pipeline:
            ...
            print("Y TRAIN SHAPE: ", Y_train_temp.shape)

            # ? Creating history of features
            history_len = 3
            X_train_temp, Y_train_temp, meta_train = EMG_Data.stackHistoryCatMeta_windows(x_num=X_train_temp,
                                                                                    y_num=Y_train_temp,
                                                                                    x_cat=X_train_cat,
            ...
            print(f"Stacked x_train feature shape: {X_train_temp.shape}")
            print(f"Stacked y_train feature shape: {Y_train_temp.shape}")


        #? Scale output features -> [-1,1] for tanh

        Y_scaler, Y_train_temp, Y_test = EMG_Data.scaleFeatures_windows(train_data=Y_train_temp, 
                                                                        test_data=Y_test,
                                                                        method="MinMaxScaler",
                                                                        feature_range=(-1,1))
...
X_train_temp = np.concatenate(X_train_combined, axis=0)
Y_train_temp = np.concatenate(Y_train_combined, axis=0)
...
all_rmse_e, all_rmse_sf, all_rmse_ss = [], [], []
all_r2_e, all_r2_sf, all_r2_ss = [], [], []

all_rmse_e_pf, all_rmse_sf_pf, all_rmse_ss_pf = [], [], []
all_r2_e_pf, all_r2_sf_pf, all_r2_ss_pf = [], [], []
...
for fold, (train_idx, val_idx) in enumerate(kf.split(X_train_temp)):
    ...
    all_r2_e.append(r2_elbow)
    all_rmse_e.append(rmse_elbow)

    all_r2_sf.append(r2_front)
    all_rmse_sf.append(rmse_front)

    all_r2_ss.append(r2_side)
    all_rmse_ss.append(rmse_side)
    ...
    all_r2_e_pf.append(r2_elbow_pf)
    all_rmse_e_pf.append(rmse_elbow_pf)

    all_r2_sf_pf.append(r2_front_pf)
    all_rmse_sf_pf.append(rmse_front_pf)

    all_r2_ss_pf.append(r2_side_pf)
    all_rmse_ss_pf.append(rmse_side_pf)


print("The results across seed are...\n")
print(f"Elbow R2: {all_r2_e_pf}")
print(f"Front R2: {all_r2_sf_pf}")
print(f"Side R2: {all_r2_ss_pf}\n")

print(f"Elbow R2 stats: Mean: {np.mean(all_r2_e_pf)}  Std. : {np.std(all_r2_e_pf)}")
print(f"Front R2 stats: Mean: {np.mean(all_r2_sf_pf)}  Std. : {np.std(all_r2_sf_pf)}")
print(f"Side R2 stats: Mean: {np.mean(all_r2_ss_pf)}  Std. : {np.std(all_r2_ss_pf)}\n")
